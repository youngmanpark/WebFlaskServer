...
def return_json_data(url):
    ...
    def sqltest(url):
        ...
        url_list = ["http://" + url + "/members/login", "http://" + url + "/login.php"]

        pass_flg = False

        def check_url_notexistence(url):
            response = requests.get(url)
            if response.status_code == 200: #url존재한다면
                return True
            else:# 존재하지 않는다면
                return False

        for url in url_list:
            if check_url_notexistence(url):# 존재한다면
                pass_flg = True #flag true
                break
            else:
                continue
        ...
        if not pass_flg:
            print(pass_flg)
            contents="There are no pages to inspect"
            is_cve = "null"
            return (module_name, description, purpose, security_threat, contents.strip(), is_cve)
    ...
    for strfunc in func_name:
        func = eval(strfunc)
        returns = func
        temp_dict = {"vulnerability": 1, "description": 1, "purpose": 1, "security_threat": 1, "content": 1,
                     "status": 1}
